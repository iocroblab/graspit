...
VariableSet::getVariable(QString name)
{
  ...
  DBGP("Requested variable " << name.toUtf8().constData() << " not found in search state");
  return NULL;
}
...
VariableSet::setParameter(QString name, double value)
{
  ...
  if (it == mParameters.end()) {
    DBGA("Parameter " << name.toUtf8().constData() << " not found!");
    assert(0);
    return;
  }
  it->set(value);
...
VariableSet::addParameter(QString name, double value)
{
  ...
  if (it != mParameters.end()) {
    DBGA("Parameter " << name.toUtf8().constData() << " already present!");
    assert(0);
    return;
  }
...
VariableSet::removeParameter(QString name)
{
  ...
  if (it == mParameters.end()) {
    DBGA("Parameter " << name.toUtf8().constData() << " does not exist!");
    assert(0);
    return;
  }
...
VariableSet::print() const
{
  ...
  for (int i = 0; i < getNumVariables(); i++) {
    fprintf(stderr, "%s = %.2f; ", mVariables[i]->getName().toUtf8().constData(), mVariables[i]->getValue());
  }
  fprintf(stderr, "\n");
}
//------------------------------- POSTURE STATES -----------------------------------
