...
KinematicChain::initChainFromXml(const TiXmlElement *root, QString &linkDir)
{
  ...
  for (l = 0, p = elementList.begin(); p != elementList.end(); p++, l++) {
    ...
    linkVec[l] = new Link(owner, chainNum, l, owner->getWorld(), linkName.toUtf8().constData());
...
KinematicChain::cloneFrom(const KinematicChain *original)
{
  ...
  for (int l = 0; l < numLinks; l++) {
    ...
    QString linkName =  QString(owner->getName()) + QString("_chain%1_link%2").arg(chainNum).arg(l);
    linkVec[l] = new Link(owner, chainNum, l, owner->getWorld(), linkName.toStdString().c_str());
