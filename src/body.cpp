...
{
  breakContacts();
  DBGP("Deleting Body: " << myName.toUtf8().constData());
}
...
Body::loadFromXml(const TiXmlElement *root, QString rootPath)
{
  ...
  if (!element) {
    ...
    if (mGeometryFileType == "Inventor") {
      ...
      DBGA("Unknown geometry file type: " << mGeometryFileType.toUtf8().constData());
      result = FAILURE;
    }
    if (result == FAILURE) {
...
Body::saveToXml(QTextStream &xml) {
  xml << "\t\t\t<material>" << myWorld->getMaterialName(material).toUtf8().constData() << "</material>" << endl;
  if (youngMod > 0) {
  ...
  xml << "\t\t\t<geometryFile type = \"" << mGeometryFileType.toUtf8().constData() << "\">"
      << mGeometryFilename.toUtf8().constData() << "</geometryFile>" << endl;
  return SUCCESS;
}
...
{
  SoInput myInput;
  if (!myInput.openFile(filename.toUtf8().constData())) {
...
{
  std::ifstream inFile(fn.toUtf8().constData(), std::ios::in);
  if (!inFile.is_open())
  {
