...
bool DBaseBatchPlanner::processArguments(int argc, char **argv)
{
  ...
  if (mType == GRIPPER) {
    ...
    FILE *tf = fopen(testfile.toUtf8().constData(), "a");
    if (!tf) {
      DBGAF(mLogStream, "Failed scan directory test: " << testfile.toUtf8().constData());
      return false;
    }
    fprintf(tf, "Starting object: %s\n", mObject->getName().toUtf8().constData());
    fclose(tf);
  }
...
bool DBaseBatchPlanner::startPlanner()
{
  ...
  if (mType == DEXTEROUS) {
    ...
    if (numChildren <= 0) {
      DBGAF(mLogStream, "Can not understand NUMBER_OF_PROCESSORS: " << numCPU.toUtf8().constData());
    }
#else
...
void DBaseBatchPlanner::writeCloudToFile(int i, int j, const std::vector<position> &cloud)
{
  ...
  FILE *f = fopen(filename.toUtf8().constData(), "w");
  if (!f) {
    DBGAF(mLogStream, "Failed to open file " << filename.toUtf8().constData());
