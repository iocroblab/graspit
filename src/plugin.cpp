...
PluginCreator *PluginCreator::loadFromLibrary(std::string libName)
{
  ...
  if (filename.startsWith("/")) {
    ...
    QFile file(filename);
    if (!file.exists()) {
      DBGA("Could not find absolute plugin file " << filename.toUtf8().constData());
      return NULL;
    }
  } else {
    ...
    if (!found) {
      DBGA("Could not find relative plugin file " << filename.toUtf8().constData() <<
  ...
  PLUGIN_DYNLIB_HANDLE handle = PLUGIN_DYNLIB_OPEN(filename.toUtf8().constData());
  char *errstr = PLUGIN_DYNLIB_ERROR();
  if (!handle) {
    DBGA("Failed to open dynamic library " << filename.toUtf8().constData());
  ...
  if (PLUGIN_DYNLIB_ERROR()) {
    DBGA("Could not load symbol createPlugin from library " << filename.toUtf8().constData());
    return NULL;
  }
  ...
  if (PLUGIN_DYNLIB_ERROR()) {
    DBGA("Could not load symbol getType from library " << filename.toUtf8().constData());
    return NULL;
  }
  ...
  if (type.empty()) {
    DBGA("Could not get plugin type from library " << filename.toUtf8().constData());
  }
