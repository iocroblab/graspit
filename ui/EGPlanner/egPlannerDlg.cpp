...
void EigenGraspPlannerDlg::init()
{
  ...
  varGridLayout = new QGridLayout(variableBox->layout()->widget());
  varGridLayout->setSpacing(5);
  varGridLayout->setAlignment(Qt::AlignTop);
  varGridLayout->addWidget(spaceSearchLabel, 0, 0, 0, 1);
  varGridLayout->addWidget(spaceSearchBox, 0, 0, 2, 4);
...
//    varGridLayout->removeItem(varNames[i]);
...
void EigenGraspPlannerDlg::inputLoadButton_clicked()
{
  ...
  FILE *fp = fopen(fn.toUtf8().constData(), "r");
  bool success = true;
  if (!fp) {
