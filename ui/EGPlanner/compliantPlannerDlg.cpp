...
CompliantPlannerDlg::updateOut()
{
  ...
  mOut = new std::fstream(filename.toUtf8().constData(), std::fstream::out | std::fstream::app);
  if (mOut->fail()) {
    DBGA("Failed to open file " << filename.toUtf8().constData());
    delete mOut; mOut = NULL;
    resultsBox->setCurrentIndex(0);
  } else {
    DBGA("Output file opened: " << filename.toUtf8().constData());
  }
}
